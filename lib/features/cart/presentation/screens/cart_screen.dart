...
class CartScreen extends ConsumerWidget {
  ...
  Future<void> _checkout(
    ...
    // Loading dialog while we ask the server for a Stripe Checkout session.
    showDialog(
      context: context,
    ...
    try {
      // The server recomputes the amount from canonical data, creates a pending
      // order, and returns the Stripe-hosted payment URL. The client never sends
      // or trusts a price — and never marks the order paid (the webhook does).
      final session = await ref
          .read(checkoutServiceProvider)
          .startCheckout(promoCode: promo?.code);

      if (!context.mounted) return;
      Navigator.of(context).pop(); // close loader

      // Land on the result screen which listens to the order until the Stripe
      // webhook flips it to `paid` (covers the web redirect and the mobile case).
      context.go('/checkout/success?order=${session.orderId}');
      ...
      _toast(context, e.message ?? 'Paiement impossible pour le moment.');
    } catch (e) {
      if (!context.mounted) return;
      Navigator.of(context).pop();
      _toast(context, 'Erreur : $e');
    }
  }
